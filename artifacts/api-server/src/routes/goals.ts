...
router.get("/goals", authMiddleware, async (req, res) => {
  ...
  const goals = await db.select().from(goalsTable).where(eq(goalsTable.userId, user.id));
...
router.patch("/goals/:id", authMiddleware, async (req, res) => {
  ...
  if (d.currentAmount !== undefined && d.currentAmount !== null) {
    ...
    updates.isCompleted = parseFloat(String(d.currentAmount)) >= parseFloat(existing[0].targetAmount);
  }
